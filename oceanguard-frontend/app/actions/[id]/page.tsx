...
export default function ActionDetailsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // try to fetch metadata JSON if the URI refers to IPFS
      try {
  ...
  // 尝试从 IPFS 读取 metadata（若是 ipfs://CID），此处简单展示链接与原始值
  ...
  return (
    <main className="container" style={{ paddingTop: '40px', paddingBottom: '80px', maxWidth: '900px' }}>
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ color: 'var(--text-dim)' }}>View on-chain record and submitted metadata</p>
      </div>
